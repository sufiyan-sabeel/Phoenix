...
def main():
    ...
    # 5. Mobile Device Remote Control Setup (ADB)
    ...
    os_platform = "Windows" if sys.platform == "win32" else ("macOS" if sys.platform == "darwin" else ("Termux" if is_termux else "Linux"))
    ...
    if sz_choice in ["y", "yes"]:
        if shutil.which("adb"):
            ...
            print(f"{YELLOW}[!] Note: 'adb' executable was not detected in PATH.{NC}")
            print(f"{YELLOW}    Install Android platform-tools and ensure 'adb' is in your PATH to enable device control.{NC}")
            shizuku_enabled = False
